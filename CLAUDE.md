...
Els contenidors de les pàgines d'Admin també es van eixamplar (`Dashboard` `max-w-4xl`→`max-w-6xl`; `ProjectIndex`/`SectionIndex`/`QuestionIndex`/`UserIndex` `max-w-3xl`→`max-w-5xl`) perquè l'usuari va detectar que en tauleta horitzontal el contingut no aprofitava l'amplada disponible (molt marge buit als dos costats). Els modals (`max-w-md`) es queden igual — no té sentit que un formulari estret s'eixampli.
